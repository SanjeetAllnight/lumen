...
const AuthForm = () => {
  ...
  const title = `Create ${role === 'admin' ? 'Admin' : 'Student'} Account`;
      ...
      <div className="landing-glass-panel w-full max-w-md p-8 md:p-10 rounded-[2.5rem] flex flex-col items-center shadow-2xl relative z-10 border border-glassBorder mt-8 md:mt-0">
        ...
        <form className="w-full flex gap-5 flex-col" onSubmit={handleSubmit}>
          <div className="flex flex-col gap-1.5 w-full">
            <label className="text-sm font-heading font-medium text-textMain/90 ml-1">Full Name</label>
            <input value={name} onChange={e => setName(e.target.value)} type="text" placeholder="Jane Doe" required className="w-full bg-surface border border-border rounded-2xl px-4 py-3 text-textMain placeholder:text-textMuted/50 focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-all duration-300" />
          </div>
          ...
          <div className="flex flex-col gap-1.5 w-full">
            <label className="text-sm font-heading font-medium text-textMain/90 ml-1">Confirm Password</label>
            <div className="relative">
              <input value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} type={showConfirm ? "text" : "password"} placeholder="••••••••" required className="w-full bg-surface border border-border rounded-2xl px-4 py-3 text-textMain placeholder:text-textMuted/50 focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-all duration-300" />
              <button type="button" onClick={() => setShowConfirm(!showConfirm)} className="absolute right-4 top-1/2 -translate-y-1/2 text-textMuted hover:text-textMain transition-colors">
          ...
          <button type="submit" disabled={isLoading} className="w-full bg-primary hover:bg-opacity-90 text-white font-heading font-semibold text-base py-3.5 rounded-2xl mt-4 transition-all duration-300 shadow-md shadow-primary/20 flex items-center justify-center gap-2 group disabled:opacity-60 disabled:cursor-not-allowed">
            {isLoading ? <span className="material-symbols-outlined animate-spin text-xl">progress_activity</span> : <>Create Account <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" /></>}
          </button>
        </form>
        ...
        <p className="mt-8 text-sm font-body text-textMuted text-center">
          ...
          <Link href={`/login?role=${role}`} className="text-primary font-medium hover:underline focus:outline-none transition-colors">Sign In</Link>
        </p>
      </div>
    </div>
  );
